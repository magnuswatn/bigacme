...
def lb(opt_username, opt_password, opt_lb, opt_datagroup, opt_partition):
    configtp = namedtuple("Config", ["lb_user", "lb_pwd", "lb1", "lb2", "lb_dg",
                                     "lb_dg_partition"])
    config = configtp(lb_user=opt_username, lb_pwd=opt_password, lb1=opt_lb, lb2=None,
                      lb_dg=opt_datagroup, lb_dg_partition=opt_partition)
...
def test_send__and_remove_challenge(lb, rest_lb, opt_partition, opt_datagroup):
    ...
    datag = rest_lb.tm.ltm.data_group.internals.internal.load(partition=opt_partition,
                                                              name=opt_datagroup)
...
def test_upload_certificate_nonexisting_partition(lb):
    cert = _generate_certificate(0, 9999999)
    with pytest.raises(bigacme.lb.PartitionNotFoundError):
        lb.upload_certificate('NotAPartition', 'test_upload_certificate_certificate', [cert])
